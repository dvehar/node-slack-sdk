 ...
 * @param {Object=} opts
 * @param {?} opts.validate - If false, modify an invalid channel name instead of return an error.
 * @param {function=} optCb Optional callback, if not using promises.
 */
ChannelsFacet.prototype.create = function create(name, opts, optCb) {
  var requiredArgs = {
    name: name
  };

  return this.makeAPICall('channels.create', requiredArgs, opts, optCb);
};
